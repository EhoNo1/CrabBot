...
difference(){
    union(){
        ...
        translate([bit_distance,0,-(back_thickness-spacing)+backing_thickness/2]) 
            cylinder(r=pin_socket*2,h=backing_thickness*back_thickness-spacing,$fn=quality);
        translate([0,-pin_socket*2,0]) 
            cube([bit_distance,pin_socket*4,backing_thickness]);  
    }
